...
router
  ...
  .post(verifyAuthToken, async function (req, res) {
    let data = R.pick(["name", "redirectURLs", "scope"], req.body);
    ...
    try {
      let projectAfterSave = await project.save();
      res.send(
        R.pick(
          ["projectID", "projectSecret", "name", "redirectURLs", "scope"],
          projectAfterSave
        )
      );
    } catch (e) {
  ...
  .get(verifyAuthToken, async function (req, res) {
    let project = await Project.find({ createdBy: req.user._id });
    res.send(project);
  })
